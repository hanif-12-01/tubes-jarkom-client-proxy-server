    ...
    try:
        ...
        if os.path.exists(filename) and not os.path.isdir(filename):
            ...
            if os.path.exists("status/404.html"):
                ...
                content_size = len(b"File Not Found")
        ...
        # Log: IP, waktu, path, status, ukuran data, waktu proses
        print(f"[{timestamp}] LOG: {address[0]} | {path} | {status_code} | {content_size} bytes | {duration:.2f} ms")
        if response:
